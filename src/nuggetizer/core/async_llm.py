...
class AsyncLLMHandler:
    def __init__(
        ...
        if resolved_api_type:  # User explicitly provided api_type
            ...
            "gpt" in model.lower() or api_type == "azure"
        ):  # existing azure condition
        ...
        if self.api_type == "openrouter":
            ...
            if resolved_api_keys:
                ...
                if not openrouter_key:
                    raise ValueError(
                        "OpenRouter API key not found. Please set OPENROUTER_API_KEY environment variable or pass api_keys."
                    )
            ...
            if (
                not self.api_base or not self.api_version
            ):  # Check if essential Azure args are present
                azure_args = (
                    get_azure_openai_args()
                )  # This function raises error if not set
            ...
            if resolved_api_keys:
                ...
                if not openai_key:
                    raise ValueError(
                        "OpenAI API key not found. Please set OPENAI_API_KEY environment variable or pass api_keys."
                    )
        ...
        if (
            not self.api_keys or not self.api_keys[0]
        ):  # Ensure api_keys list is not empty and first key is valid
    ...
    def _initialize_client(self):
        if self.api_type == "azure":
            ...
            return AsyncOpenAI(api_key=self.api_keys[self.current_key_idx])
        elif self.api_type == "openrouter":
            return AsyncOpenAI(
                api_key=self.api_keys[self.current_key_idx], base_url=self.api_base
            )
        else:
            raise ValueError(
                f"Invalid API type for client initialization: {self.api_type}"
            )
