...
try:
    ...
    GET_OPENROUTER_API_KEY_PATH = "src.nuggetizer.core.llm.get_openrouter_api_key"
    GET_ASYNC_OPENROUTER_API_KEY_PATH = (
        "src.nuggetizer.core.async_llm.get_openrouter_api_key"
    )
    ...
    GET_OPENROUTER_API_KEY_PATH = "nuggetizer.core.llm.get_openrouter_api_key"
    GET_ASYNC_OPENROUTER_API_KEY_PATH = (
        "nuggetizer.core.async_llm.get_openrouter_api_key"
    )
    TIKTOKEN_PATCH_PATH = "tiktoken.get_encoding"
...
class TestLLMHandlerOpenRouter(unittest.TestCase):
    ...
    def test_openrouter_initialization_with_custom_base_url(self):
        ...
        self.MockOpenAIConstructor.assert_called_once_with(
            api_key="test_openrouter_key_from_env", base_url=custom_url
        )
    ...
    def test_openrouter_initialization_with_direct_api_key(self):
        ...
        LLMHandler(model="openrouter/some-model", api_keys=direct_key)
    ...
    def test_openrouter_run_method(self):
        ...
        self.mock_openai_client_instance.chat.completions.create.return_value = (
    ...
    def test_openrouter_api_key_rotation(self):
        api_keys = ["key1", "key2"]
        handler = LLMHandler(
            model="openrouter/my-model", api_type="openrouter", api_keys=api_keys
        )

        self.assertEqual(handler.api_keys, api_keys)
        self.MockOpenAIConstructor.assert_called_with(
            api_key="key1", base_url="https://openrouter.ai/api/v1"
        )
        ...
        self.mock_openai_client_instance.chat.completions.create.side_effect = [
            ...
            MagicMock(
                choices=[MagicMock(message=MagicMock(content="Success on key2"))]
        ...
        self.assertEqual(
            self.mock_openai_client_instance.chat.completions.create.call_count, 2
        )
        self.assertEqual(handler.current_key_idx, 1)  # Key index should have advanced
        self.assertEqual(
            handler.client.api_key, "key2"
        )  # Client api_key property updated
...
class TestAsyncLLMHandlerOpenRouter(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):  # Renamed for clarity, unittest calls this if defined
        self.get_key_patcher = patch(GET_ASYNC_OPENROUTER_API_KEY_PATH)
        self.mock_get_openrouter_api_key = self.get_key_patcher.start()
        self.mock_get_openrouter_api_key.return_value = (
            "test_async_openrouter_key_from_env"
        )
        ...
        self.mock_async_openai_client_instance = (
            AsyncMock()
        )  # Use AsyncMock for async methods
        self.mock_async_openai_client_instance.chat.completions.create = (
            AsyncMock()
        )  # create is async
        self.MockAsyncOpenAIConstructor.return_value = (
            self.mock_async_openai_client_instance
        )
    ...
    async def test_openrouter_initialization_with_custom_base_url_async(self):
        ...
        handler = AsyncLLMHandler(
            model="openrouter/some-async-model", api_base=custom_url
        )
        self.mock_get_openrouter_api_key.assert_called_once()
        self.MockAsyncOpenAIConstructor.assert_called_once_with(
            api_key="test_async_openrouter_key_from_env", base_url=custom_url
        )
        self.assertEqual(handler.api_base, custom_url)

    async def test_openrouter_initialization_with_direct_api_key_async(self):
        direct_key = "direct_async_or_key_456"
        AsyncLLMHandler(model="openrouter/some-async-model", api_keys=direct_key)
        self.mock_get_openrouter_api_key.assert_not_called()
        self.MockAsyncOpenAIConstructor.assert_called_once_with(
            api_key=direct_key, base_url="https://openrouter.ai/api/v1"
        )
    ...
    async def test_openrouter_run_method_async(self):
        ...
        mock_completion_response = (
            MagicMock()
        )  # AsyncMock isn't strictly needed for the response object itself
        ...
        self.mock_async_openai_client_instance.chat.completions.create.return_value = (
            mock_completion_response
        )
    ...
    async def test_openrouter_api_key_rotation_async(self):
        ...
        handler = AsyncLLMHandler(
            model="openrouter/my-async-model", api_type="openrouter", api_keys=api_keys
        )

        self.assertEqual(handler.api_keys, api_keys)
        self.MockAsyncOpenAIConstructor.assert_called_with(
            api_key="async_key1", base_url="https://openrouter.ai/api/v1"
        )
        self.mock_async_openai_client_instance.chat.completions.create.side_effect = [
            Exception("Simulated API error on async_key1"),
            MagicMock(
                choices=[MagicMock(message=MagicMock(content="Success on async_key2"))]
            ),
        ]
        ...
        self.assertEqual(handler.client.api_key, "async_key2")
        self.assertEqual(response, "Success on async_key2")
        self.assertEqual(
            self.mock_async_openai_client_instance.chat.completions.create.await_count,
            2,
        )
